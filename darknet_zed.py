...
def main(argv):
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            # Do the detection
            start_time = time.time()  # start time of the loop - checkpoint
            detections = detect(netMain, metaMain, image, thresh) # detections performed and obtained from yolo framework
            bench_time = time.time()  # setting checkpoint for the loop - checkpoint
            ...
            for detection in detections:
                ...
                cropped_image = image[y_coord:(y_coord + y_extent), x_coord:(x_coord + x_extent)] # cropped image for image comparison
                detected_objects = Bridge.get_detected_objects(detected_objects, label, x, y, z, camera_pose,py_translation, cropped_image, existing_labels, positional_buffer_array, rotational_buffer_array, rotation_timer)
            ...
            Bridge.opfileprint(str(detection_write)) # writing detection data for the frame into file "YOLO_OUTPUT.txt"
