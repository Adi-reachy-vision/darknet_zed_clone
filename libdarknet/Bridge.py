...
def get_detected_objects(detected_objects, label, x, y, z, camera_pose, py_translation, cropped_image, existing_labels,
    ...
    tx, ty, tz, rx, ry, rz, stable = positional_buffer_CAMERAframe(camera_pose, py_translation,
    ...
    if tx != 0.0 or ty != 0.0 or tz != 0.0:  # update only when translational value is greater than zero
        for detected_rot in detected_objects: #updating values based on
...
def positional_data(camera_pose, py_translation):
...
def positional_buffer_CAMERAframe(camera_pose, py_translation, positional_buffer_array, rotational_buffer_array):
    tx, ty, tz, rx, ry, rz = positional_data(camera_pose, py_translation)
    ...
    return tx, ty, tz, rx, ry, rz, stable  # returning translation and rotation array
