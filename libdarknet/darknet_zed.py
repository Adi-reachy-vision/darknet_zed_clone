    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            if tracking_state == sl.POSITIONAL_TRACKING_STATE.OK: #activates only when the poisitional tracking state is in 'OK' state
                text_translation, text_rotation = get_positional_data(camera_pose, py_translation) #gets translation and rotation data as a string
            ...
            for detection in detections:
                ...
                detected_o = [label, int(bounds[0]), int(bounds[1]), round(x, 3), round(y, 3), round(z, 3)]
                ...
                '''for detected in detected_objects:
                    if label == detected[0]:
                        if int(bounds[0]) == abs(detected[1]):
                            detected_o = [label + str(count_o), int(bounds[0]), int(bounds[1]), round(x, 3),
                                          round(y, 3), round(z, 3)]
                            detected_objects.remove(detected)
                            detected_objects.append(detected_o)
                        elif int(bounds[1]) == abs(detected[2]):
                            detected_o = [label + str(count_o), int(bounds[0]), int(bounds[1]), round(x, 3),
                                          round(y, 3), round(z, 3)]
                            detected_objects.remove(detected)
                            detected_objects.append(detected_o)
